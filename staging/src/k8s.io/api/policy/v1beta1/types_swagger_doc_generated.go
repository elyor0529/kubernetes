...
var map_AllowedHostPath = map[string]string{
	...
	"readOnly":   "when set to true, will allow host volumes matching the pathPrefix if all volume mounts are readOnly. If the container specifically requests to run with a non-read only volume that matches the path prefix then the PSP should deny the pod. If set to false the container may run with a read-only root volume if it wishes but it will not be forced to.",
}
